...
class Sigmoid:
    ...
    def __init__(self, weights = np.array([1])):
        ...
        # define the function to compute the logistic function
    def logistic(self,x):
        return 1/(1+np.exp(-x))
    ...
    def update(self, values, train, eta=.1):
        ...
        for X, y_true in zip(values, train):
            ...
            dlogisticdx = self.logistic_derivative(np.dot(X,self.weights))
            # TODO: update self.weights based on learning rate, signal accuracy,
            # function slope (derivative) and input value
            deltaWeight = eta*(y_true - y_pred)*dlogisticdx*X
            self.weights = self.weights + deltaWeight
